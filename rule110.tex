...
\begin{document}
    ...
    \begin{abstract}
        ...
        input and output transaction states.\dnote{Don't like `minimalistic`
    ...
    universality. For example, in~\cite{buterin2014next} authors introduced the
    $jump$ opcode in the transaction scripting language in order to incorporate
    loops. This resulted in several formal and security
    complications\vk{citations for gas and security issues}. However,
    the main obstacles to the formal Turing completeness in the bolckchain,
    namely potentially infinite execution time, and potentially infinite storage
    memory, still persist.
    ...
    introduced by E.~L.~Post, and are shown to be capable of emulating universal
    Turing machines~\cite{post1943formal}\dnote{remove last sentence?} \vk{Maybe
        reformulate then. It would be good to keep the full chain of proofs to
    highlight the formal correctness}.
    ...
    For description of Rule 110 implementation in a blockchain, we use Bitcoin--like
    ...
    constructed over the byte array containing the new outputs scripts. However,
    ...
    script structure:
    \begin{eqnarray*}
        isRule110Transformation(self.\payload{}, output[0].\payload{}) \And{} \\
    ...
    %    \end{algorithmic}
    %\end{algorithm}
    The function $isRule110Transformation$ returns $true$ if and only if the second argument
    is the one step Rule 110 transformation of the first argument. It may be
    implemented in a number of ways based on scripting language features. In
    \nameref{appendix1} we provide the full script implementing the rule 110 in
    the scripting language of the Ergo blockchain.
    ...
    % можно определить функцию проверки одного бита, и затем вызывать ее нужное количество раз для каждого бита в слое, типа check110(0) && check110(1) && check110(2) && check110(3) && check110(4) && check110(5)
    %    \begin{algorithm}[H]
    ...
    \begin{figure}[h]
        ...
        \caption{Transaction chain following Rule 110. See the text for the
    ...
    of the automaton. This implementation, while being inefficient, keeps
    ...
    system can be achieved by combining multiple transactions together. \vk{Note
        that the language requirements are not demanding, and they are met in a
    number of platforms.}
