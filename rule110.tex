...
Research Institute\\\email{v.kharin@protonmail.com}}
    ...
    time for a transaction must be bounded as well, otherwise blockchain could
    trivially get stuck.
...
%    to \nameref{appendix1}.
    ...
    \begin{eqnarray}
        (\wedge_i\wedge_j(out[i].x_j=f_{ij}(in)))\wedge \\ 
        (\wedge_i in[1].x_i = g_{1i}(in[0]))\wedge(\wedge_i in[2].x_i =
        g_{2i}(in[0],in[2]))\wedge\dots
