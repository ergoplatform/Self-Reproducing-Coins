...
\begin{document}
	...
	Moreover, Turing--completeness of this system is still a %This - Etherium or Bitcoin Script?
    subject of debates mostly due to undecidability of the halt problem.  For % Судя по Википедии, halt problem is undecidable over Turing machines, т.о. не может служить доказательством не-Тьюринговорсти
    ...
    the real--world blockchain applications. \nameref{appendix1} sketches the discussion on the
	...
	The script works as follows. Every output contains its bit value $val$, % as follows более употребительный варинант
    ...
    that our language requirements are not very demanding, and can be met in a % какие именно требования языку - не описано. Может быть это можно формалиовать?
    ...
    a couple of transactions if needed. Thus, the hard computation limit per block % a couple of или multiple
    may be exceeded by chaining multiple transactions between different blocks. % может быть exceeded->surpassed? Смысл больше в сторону "преодолен", а не "упс, превышен"
