...
Research Institute\\\email{v.kharin@protonmail.com}}
    ...
    recursion. By doing this we let a program to be executed over a set of
    ...
    A single transaction in the blockchain is approximately corresponding to a single cycle of a computing 
    ...
    One can wonder how evolving data structures~(a blockchain and a corresponding UTXO set) 
    along with programmable validation rules constitute a Turing machine. Obviously, we do need 
    to include honest miners as well, their efforts are making the input tape of the machine. 
    The same is true for Ethereum and other blockchains with Turing-complete smart contracts: 
    the blockchain as a data structure is not doing active computations, it is storing their result, 
    if the computations are valid according to some rules. Chain quality property from~\cite{garay2015bitcoin} 
    should hold for a blockchain protocol in order to have a guarantee on input presence. 
