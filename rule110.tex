...
Research Institute\\\email{v.kharin@protonmail.com}}
    ...
    \begin{abstract}
        ...
        In opposite, we show that Turing completeness of a blockchain system can %In opposite как-то режет слух. On the contrary? Instead? Despite this belief?
    ...
    requred for Turing completeness, can be modeled by chaining potentially infinite 
    number of transactions in the blockchain. However, there is one pitfall left.
	...
	operations. Our construction is not needed for that, it just % тут "для нашего доказательства этого не требуется?" - тогда "Our construction doesn't require this". Или "для практических применений не обязательно использовать наш конструкт" - тогда может быть что-то типа "...arithmetic operations, and it does not necessarily need to use our construction". It is just used to prove that any algorithm..."
    ...
    proofs of various smart contract languages, e.g. it might be possible to prove % А в Waves есть функции для вашего скрипта? Если есть, то доказательство тривиально и можно написать более определено напр. "e.g. we can prove that ..."
	...
	Unbouded loops power in terms of variety of algorithms that may be implemented
    inside one transaction can't be fully utilised in blockchain systems because of 
