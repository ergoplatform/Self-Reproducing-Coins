...
Research Institute\\\email{v.kharin@protonmail.com}}
    ...
    needed for Turing complete systems, can be modeled by chaining potentially infinite 
    number of transactions in the blockchain. However, there is a pitfall left.
    The size of the data stored in an output must have an upper-bound, and validation time for a transaction must be bounded as well, otherwise blockchain could trivially stuck.
